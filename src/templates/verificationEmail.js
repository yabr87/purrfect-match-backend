...
const htmlTemplate = {
  en: (verificationToken, otp, validUntil) => `
    ...
    <ol>
      ...
      <li><a target="_blank" href="${FRONTEND_URL}?verificationToken=${verificationToken}&otp=${otp}">Сlick here and proceed to the Purrfect-match website. Then request to verify your email will be sent'</a></li>
  ...
  ukr: (verificationToken, otp, validUntil) => `
    ...
    <ol>
      ...
      <li><a target="_blank" href="${FRONTEND_URL}?verificationToken=${verificationToken}&otp=${otp}">Натисніть сюди та перейдіть на сайт Purrfect-match. Потім буде відправлено запит для підтвердження вашої пошти.'</a></li>
